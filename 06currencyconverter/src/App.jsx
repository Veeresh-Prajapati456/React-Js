...
function App() {
  ...
  const options = Object.keys(currencyInfo); // to get only the keys of currency object
  ...
  const convert = () => {
    ...
    setConvertedAmount(Number(calculatedAmount.toFixed(3)));
  };

  const handleFocus = () => {
    console.log("Input focused:", amount);
    if(amount === 0){
      setAmount('');
    }
  }
  ...
  const handleBlur = () => {
    console.log("Input blurred:", amount);
    if(amount === ''){
      setAmount(0);
    }
  }
